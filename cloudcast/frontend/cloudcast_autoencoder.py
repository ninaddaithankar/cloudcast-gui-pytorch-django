...
# !pip install pytorch-lightning kaggle pyresample cartopy shapely

# !mkdir ~/.kaggle #create the .kaggle folder in your root directory
# !echo '{"username":"ninaddaithankar","key":"a92bb263d676210d84270c5c9b4cd6fb"}' > ~/.kaggle/kaggle.json #write kaggle API credentials to kaggle.json
# !chmod 600 ~/.kaggle/kaggle.json  # set permissions

# !kaggle datasets download -d tanaym/cloud-dataset

# !mkdir cloudcast-dataset
# !unzip cloud-dataset -d cloudcast-dataset
...
def make_one_hot(labels, C=4):
    ...
    one_hot = torch.cuda.FloatTensor(labels.size(0), C, labels.size(
...
def save_images_cartopy(prediction_video, ground_truth_video, target_times, batch, lines=False, high_res_map=True, path="/data/output/"):
    ...
    create_video(prediction_video, 'Prediction', target_times,
                 batch, crs, lines, high_res_map, path)
    create_video(ground_truth_video, 'Output', target_times,
                 batch, crs, lines, high_res_map, path)


def create_video(video, name, target_times, batch, crs, lines=False, high_res_map=False, path="/data/output/"):
    ...
    for frame in np.arange(0, video.shape[1]):
        ...
        plt.savefig(path + 'batch_{}/'.format(batch) + name + '_' +
                    str(target_times[0, frame])[0:16] + '_frame_' + str(frame) + '.png')
        ...
        for t in range(seq_len):
            if future:
                ...
                if metachannels is not None:
                    output = torch.cat(
                        [output, metachannels[:, t, :, :].unsqueeze(1).cuda()], 1)
...
def save_predictions(trainer, model, save_images_path):
    ...
    for i, (imgs_X, imgs_Y, time_stamp) in enumerate(test_loader):
        ...
        save_images_cartopy(prediction_video=pred_label, ground_truth_video=y_true, target_times=target_times, batch=i,
                            lines=False, high_res_map=False, path=save_images_path)
...
parser.add_argument("--pretrained_path",
                    default='./ae-convlstm.ckpt', type=str)
...
if bool(hparams.train):
    ...
    save_images_path = './model-checkpoints/model_outputs'
